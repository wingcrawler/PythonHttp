...
response = opener.open('http://www.jhyc.cn/index_zwaction.do?method=init')
...
def oreder_request(begin_date, end_date):
    urlorder = 'http://www.jhyc.cn/elonlineindentaction.do?method=orderquery&index=32'
    # 定义查询参数
    orderdata = {
...
url = 'http://www.jhyc.cn/public/public/validimg.jsp'
...
if response:
    ...
    image_stream = open('D:\\Crawler\\OCR\\court_temp\\logigjinhua.png', 'wb')
    image_stream.write(image_data)
    image_stream.close()
    imagecode = image_linux.image_ocr('D:\\Crawler\\OCR\\court_temp\\logigjinhua.png')
    ...
    if len(imagecode) == 4:
        # 登录地址
        loginurl = 'http://www.jhyc.cn/custinfo_swaction.do?method=check'
        # 定义cookie
        filename = 'cookie.txt'
