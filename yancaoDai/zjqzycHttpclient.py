...
response = opener.open('http://www.zjqzyc.com/custinfo_swaction.in?method=init')
...
def oreder_request(begin_date, end_date):
    urlorder = 'http://www.zjqzyc.com/elonlineindentaction.do?method=order&index=32'
    # 定义查询参数
    orderdata = {
...
url = 'http://www.zjqzyc.com/public/public/validimg.jsp'
...
if response:
    ...
    image_stream = open('D:\\Crawler\\OCR\\court_temp\\loginquzhou.png', 'wb')
    image_stream.write(image_data)
    image_stream.close()
    imagecode = image_linux.image_ocr('D:\\Crawler\\OCR\\court_temp\\loginquzhou.png')
    ...
    if len(imagecode) == 4:
        # 登录地址
        loginurl = 'http://www.zjqzyc.com/custinfo_swaction.do?method=check'
        # 定义cookie
        filename = 'cookie.txt'
