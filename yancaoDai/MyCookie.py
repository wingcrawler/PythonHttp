...
def oreder_request(begin_date, end_date):
    urlorder = 'http://220.189.236.198/cunet/elonlineindentaction.do?method=orderquery&index=32'
    # 定义查询参数
    orderdata = {
...
if response:
    ...
    image_stream = open('loginTesttemp.png', 'wb')
    image_stream.write(image_data)
    image_stream.close()
    imagecode = image_linux.image_ocr('loginTesttemp.png')
