...
response = opener.open('http://www.zjsxyc.com')
...
def oreder_request(begin_date, end_date):
    urlorder = 'http://www.zjsxyc.com/elonlineindentaction.do?method=orderquery&index=32'
    # 定义查询参数
    orderdata = {
...
url = 'http://www.zjsxyc.com/public/public/image.jsp'
...
if response:
    ...
    image_stream = open('D:\\Crawler\\OCR\\court_temp\\logigshaoxing.png', 'wb')
    image_stream.write(image_data)
    image_stream.close()
    imagecode = image_linux.image_ocr('D:\\Crawler\\OCR\\court_temp\\logigshaoxing.png')
    ...
    if len(imagecode) == 4:
        # 登录地址
        loginurl = 'http://www.zjsxyc.com/WebLogin.do?method=check'
        # 定义cookie
        filename = 'cookie.txt'
