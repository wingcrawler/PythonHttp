...
import logging
LOG_FILE = 'logs\ChongQing_OCR.log'
...
def im_color(r,g,b):
    ...
    arr_g=array(g)
    arr_b=array(b)
    multilist =array([[255 for col in range(63)] for row in range(22)])
    for i in range(22):
        for j in range(63):
            if (arr_r[i,j]<2)and(arr_g[i,j]<2)and(arr_b[i,j]<2)and(j>15):
            	multilist[i,j]=250
            else:
		multilist[i,j]=arr_b[i,j]
...
def recognition(im):
    result = image_to_string(im,config='LETTER1')
    result=char_repalace(result)
    temp=result[0:len(result)]
    return temp

def ChongQingfileToImage(file, id):
    try:
        print file
        ...
        ima_binary = binary(ima_gray, 150)  # 二值化
